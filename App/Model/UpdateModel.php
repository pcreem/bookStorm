...
class UpdateModel implements Update
{
    ...
    public function userPurchaseOneBook(int $userId,int $storeId,int $bookId){
        try{
            ...
            $sql='SELECT * FROM StoreBook WHERE booksId=:bookId and storesId=:storeId';
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute(['storeId' => $storeId,'bookId' => $bookId]);
            $checkStoreBook = $stmt->fetchAll(PDO::FETCH_ASSOC);

            if (count($checkUser) > 0 && count($checkStoreBook) > 0){
